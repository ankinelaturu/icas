...
The MCP adapter should not duplicate browser or replay logic. On `business_outcome`, surface `heading` and `summary` from the matching `possibleOutcomes` entry (plus `match`) so the calling agent gets a formatted message. Evidence still holds the screenshot.
