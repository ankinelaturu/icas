...
Model settings are `ICAS_DISCOVERY_LLM_*` (`MODEL`, `API_KEY`, optional `BASE_URL`, sampling). `MODEL` is `provider/model` (default `openai/gpt-4o`). Leave `BASE_URL` empty for hosted OpenAI or Anthropic; set it to an OpenAI-compatible `/v1` URL for a local server. When those keys are empty, the CLI loads repo-root `.env` (so `pnpm --filter @icas/agent exec` still works). A non-empty shell export is not overwritten.
