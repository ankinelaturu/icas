...
Starts a stdio MCP server. Each catalog capability becomes one tool (`loan-payoff` → `loan_payoff`) with typed arguments from the artifact inputs plus `url` and optional `tenant`. Tool calls use `CapabilityResolver` + `ReplayEngine` (tenant must be enrolled; default `icas-bank`). On `business_outcome`, the tool text includes `heading`, `summary`, and `message` (the phrase that hit); evidence still holds the screenshot.
