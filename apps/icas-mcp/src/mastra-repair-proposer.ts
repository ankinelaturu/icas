/**
 * @file Mastra-backed RepairProposer for MCP `assist`.
 *
 * Lives in this app so `@icas/replay` stays model-free and `@icas/discovery`
 * stays the discover path (LLM settings helpers only). Same adapter as
 * `icas-play`; apps must not import each other. ReplayEngine still
