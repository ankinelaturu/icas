/**
 * @file Express app for the icas-banc synthetic core-banking UI.
 *
 * Same Vendor+Product as icas-bank (LS 4.12.08). Institution chrome and the
 * search submit label differ. Overlay (`?inject=`) still sits on loan details.
 */
...
export interface IcasBancAppOptions {
...
/**
 * Build the icas-banc request handler. Does not listen.
 *
 * @param options - Optional page/static roots for tests
 * @returns An Express app ready for `listen` or `supertest`
 */
export function createIcasBancApp(options: IcasBancAppOptions = {}): Express {
    ...
    sendHtml(
      ...
      page("account.html", {
        ...
        ...overlayTemplateVars(
          ...
          // Cookie-backed `message=` so HITL copy survives Home → Look Up.
