/**
 * @file HTTP tests for the icas-banc tenant pages.
 */
...
import { createIcasBancApp } from "../src/app.js";
...
describe("icas-banc HTTP", () => {
  ...
  it("walks Home → Lending → Look Up → Payoff → statement for loan 987654", async () => {
    const baseUrl = await listen();
    ...
    const searchForm = await (await fetch(`${baseUrl}/lending/search.htm`)).text();
    expect(searchForm).toContain('value="Look Up"');
    expect(searchForm).not.toContain('value="Inquire"');
