 ...
 * matches a compiled `possibleOutcomes` phrase.
 *
 * Wait overlay stays on **search** (step drift vs icas-bank loan details).
 * HITL overlay is on the **payoff statement** so it can match
 * `payoff-statement` `click-calculate-generate` (`Authorization required`).
 */
...
/** Query/cookie key for overlay body copy. */
...
export const DEFAULT_HITL_BODY =
  "Supervisor hold. Do not release this payoff statement until authorization is confirmed.";
